    ...
    <table class="mail-body" cellpadding="0" cellspacing="0" border="0" width="600px" align="center" style="margin:10px auto; width:600px;">
        <tr class="header-ads">
            <td style="text-align:center; vertical-align:bottom;" background="{{ asset('images/email-tmp/banner-600x260/save-now.jpg') }}" height="260px">
                <a href="{{$link}}" style="background-color:#a85dc2; border-radius:2px; padding:15px 100px; font-size:16px; font-weight:bold; color:#fff; display:inline-block; margin-bottom:60px; text-decoration:none;">Get Deal</a>
            </td>
        </tr>
        <tr class="content-ads">
            <td>
                <h3 style="color:#333; padding:15px 0 0;">{{$deal['title']}} </h3>
                @if(!empty($deal['expire_date']))
                    <p style="color:#999; font-style:italic; padding:10px 0; text-align:justify; line-height:22px; font-size:14px;">Expire: {{date_format($deal['expire_date'],"d M Y")}}</p>
                @endif
                <p style="padding:10px 0; text-align:justify; color:#666; line-height:22px; font-size:14px;">{{$deal['description']}}</p>
            </td>
        </tr>
        @if(!empty($hotCoupons))
